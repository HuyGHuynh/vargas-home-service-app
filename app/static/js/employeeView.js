// Current employee data - loaded from backend
let currentEmployee = {
    id: null,
    firstName: "",
    lastName: "",
...
// Employee jobs - loaded from API
let employeeJobs = [];

// Load employee jobs from API
async function loadEmployeeJobs(employeeId) {
    try {
        const response = await fetch(`/api/employee/${employeeId}/jobs`);
        const result = await response.json();

        if (result.success) {
            employeeJobs = result.data;
            return true;
        } else {
            console.error('Failed to load employee jobs:', result.message);
            employeeJobs = [];
            return false;
        }
    } catch (error) {
...
// Sample jobs for reference (remove after API integration)
let sampleJobs = [
    {
        id: 1,
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const employeeId = getEmployeeIdFromURL() || 1; // Default to 1 for now
    ...
    if (!jobsLoaded) {
        console.warn('Failed to load jobs from API');
    }
});

// Helper function to get employee ID from URL
function getEmployeeIdFromURL() {
    const pathParts = window.location.pathname.split('/');
    const employeeIndex = pathParts.indexOf('employee');
    if (employeeIndex >= 0 && pathParts[employeeIndex + 1]) {
...
function initializeCalendar() {
    ...
    const events = employeeJobs.map(job => ({
        ...
        className: normalizeStatus(job.status)
    }));
...
function openJobModal(jobId) {
        ...
        <div class="detail-section">
            ...
            <div class="detail-row">
                ...
                <div class="detail-value">
                    <span class="status-badge ${normalizeStatus(job.status)}">${getStatusDisplayText(job.status)}</span>
                </div>
            </div>
            <div class="detail-row">
            ...
            <div class="detail-row">
                ...
                <div class="detail-value">${formatTime(job.startTime)}</div>
            </div>
            <div class="detail-row">
            ...
            <div class="detail-row">
                ...
                <div class="detail-value">$${job.actualCost.toFixed(2)}</div>
            </div>
            ` : ''}
            <div class="detail-row">
                <div class="detail-label">Service Type:</div>
    ...
    // Show/hide cancel button based on job status (only for in-progress jobs)
...
// Status button highlight function removed since status update UI was removed
...
// Work notes functionality removed
...
function normalizeStatus(status) {
    ...
    // Map various status formats to CSS-friendly classes
    const statusMap = {
        'pending': 'pending',
...
function getStatusDisplayText(status) {
    ...
    return displayMap[normalizeStatus(status)] || status;
}
